$(document).ready(function(){
    ...
    $('#addAdmButton').on('click', function(){
        if (clickCheck == false) {
            ...
            $(".add-function").append("<form class='form' action='/painel-administrativo/secretarios' method='post'></form>");
            $(".form").append("<select name='add_id' id='select'></select>")
            $("#select").append("<option value='' disabled selected>-- Selecione um usuário--</option>")
            $.getJSON('/painel-administrativo/secretarios?is_admin=1', function(dados){
                if (dados) {
            ...
            $('#modal-body').append('Tem certeza que deseja adicionar esse secretário?')
